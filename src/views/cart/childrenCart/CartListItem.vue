<!--  -->
<template>
  <div class="cart_list_item ">
    <div class="cart-list-left">
    <checked 
      class="cart-img"
      :is-checked='cartInfo.isChecked'
      @click.native="imgClick"
    ></checked>
    </div>
    <div class="item_img">
      <img :src="cartInfo.image" alt="" />
    </div>
    <div class="item_info">
      <p class="item_title">{{ cartInfo.title }}</p>
      <p class="item_desc">{{ cartInfo.desc }}</p>
      <span class="item_price">￥{{ cartInfo.price }}</span>
      <span class="item_count">x{{ cartInfo.count }}</span>
    </div>
  </div>
</template>

<script>
import checked from 'components/common/checked/Checked'
export default {
  components: { checked, },
  name:'CartListItem',
  data () {
    return {
    }
  },
  props:{
    cartInfo:{
      type:Object,
      defalut(){
        return {}
      }
    }
  },
  methods: {
    imgClick(){
      this.cartInfo.isChecked=!this.cartInfo.isChecked
    }
  },
}
</script>

<style  scoped>
.cart_list_item {
  width: 100%;
  height: 5.5rem;
  border-bottom: 0.04rem solid rgba(128, 128, 128, 0.4);
  display: flex;
  padding: 0.4rem;
  padding-left: 0rem;
}
.cart-list-left{
  height: 100%;
  display: flex;
  flex-flow: column nowrap;
  justify-content: center;
}
.cart-img{
  width: 1.2rem;
  height: 1.2rem;
  border-radius: 10px;
  border: 0.1rem solid #ccc;
}

.item_img {
  width: 3.5rem;
  height: 100%;
}
.item_img img {
  width: 100%;
  height: 100%;
  border-radius: 0.4rem;
}

.item_info {
  position: relative;
  width: calc(100% - 3.5rem - 1.2rem - 0.4rem);
  margin-left: 0.4rem;
}
.item_info p {
  display: inline-block;
  width: 100%;
  white-space: nowrap;
  text-overflow: ellipsis;
  overflow: hidden;
}
.item_title {
  font-size: 0.75rem;
  padding-bottom: 0.5rem;
}
.item_desc {
  font-size: 0.6rem;
  color: gray;
}
.item_price {
  position: absolute;
  left: 0;
  bottom: 0.4rem;
  font-size: 0.75rem;
  color: orangered;
}
.item_count {
  position: absolute;
  right: 0.4rem;
  bottom: 0.4rem;
  font-size: 0.75rem;
}
</style>
